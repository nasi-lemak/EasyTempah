/**
 * Base seed data: demo users, menu, modifiers, combo, ingredients and tables.
 * Shared by `npm run seed` and `npm run seed:demo`.
 *
 * Demo PINs — admin: 9999, manager: 5555, cashier: 1234, kitchen: 2468
 */
import { db, newQrToken } from './connection';
import { hashPin } from '../middleware/auth';

/** Seeds the base data. Returns false (no-op) if any user already exists. */
export function seedBaseData(): boolean {
  ...
  const seed = db.transaction(() => {
    ...
    mainHall.forEach(([x, y], i) =>
      insertTable.run(`T${i + 1}`, 'Main Hall', i < 4 ? 2 : 4, x, y, 'square', newQrToken()),
    );
    const outdoor: [number, number][] = [[15, 40], [38, 40], [62, 40], [85, 40]];
    outdoor.forEach(([x, y], i) => insertTable.run(`A${i + 1}`, 'Outdoor', 4, x, y, 'round', newQrToken()));
    insertTable.run('VIP 1', 'VIP Room', 8, 30, 50, 'round', newQrToken());
    insertTable.run('VIP 2', 'VIP Room', 10, 70, 50, 'round', newQrToken());
  });
